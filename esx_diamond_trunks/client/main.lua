...
AddEventHandler('esx_vehicletrunk:onQuit', function()
	...
	ESX.ShowNotification('Item is ~g~stored~w~.')
	...
	for i = 1, #currentContent, 1 do
		if currentContent[i].name == name then
			if currentContent[i].count + count > limit then
				ESX.ShowNotification('Your Trunk is ~w~full~r~.')
				return
			else
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if count == nil or count < 1 or (take and count > itemCount) or (not take and count > itemLimit) then
				ESX.ShowNotification("~r~Invalid Quantity~r~.")
			elseif take then
				TakeItem(item, count)
			else
...
function AddItemsMenu()
	if #currentContent >= Config.MaxItems then
		ESX.ShowNotification('~w~Invalid Quantity~r~!')
		OpenTrunkMenu()
		return
	end
...
function TakeWeapon(name)
	...
	if not found then
		ESX.ShowNotification('~r~Item not found~w~.')
	else
...
function AddWeaponsMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      menu.close()
	  if #currentContent >= Config.MaxItems then
		ESX.ShowNotification('~y~Invalid Quantity~w~.')
		Wait(500)
		OpenTrunkMenu()
	  else
		ESX.UI.Menu.Open(
		...
		{
			title = "Are you sure you want to save a " .. data.current.label .. "?"
...
function InitTrunkMenu()
	...
	ESX.TriggerServerCallback('esx_vehicletrunk:openTrunk', function(trunk)
		if not trunk.error then
			...
			ESX.ShowNotification('Busy Trunk.')
		end
	end, currentPlate)
end
...
function OpenTrunkMenu()
		...
		table.insert(elements, {label = 'Add Items', value = 'add_item'})
		table.insert(elements, {label = 'Add Weapons', value = 'add_weapon'})
		
		ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'trunk_menu',
		{
		    title    = 'Trunk - ' .. currentPlate,
			align    = 'top-right',
			elements = elements
		},
		function(data, menu)
			...
			if data.current.value == nil and data.current.type == 'weapon' then
				...
				'dialog', GetCurrentResourceName(), 'failed_at_coding_dialog',
				{
					title = "Are you sure you want to take the " .. data.current.label .. "?"
